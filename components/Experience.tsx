...
icon: '🏢',
bullets: [
{ label: 'Full-Stack Architecture', desc: 'Built and deployed Zestix (Next.js, Nuxt.js) with WebSockets, Framer Motion, and a scalable dark-mode design system.' },
{ label: 'AI & RAG Systems', desc: 'Engineered a high-accuracy RAG pipeline using Supabase (pgvector) with a custom 3-tier context injection framework.' },
{ label: 'Automation & Integrations', desc: 'Developed 55+ workflows in n8n, integrating Apify, Firecrawl, and custom Puppeteer scripts to reduce costs.' },
{ label: 'DevOps & Infrastructure', desc: 'Managed Linux VPS production with Docker, Traefik, and Redis-based API rate limiting.' },
{ label: 'Analytics & Optimization', desc: 'Built a GEO audit tool and automated GA4/Search Console reporting for 220+ client sites.' },
],
...
icon: '🔬',
bullets: [
{ label: 'AI Model Development', desc: 'Developed a highly efficient AI model for early skin cancer detection, optimized for mobile and low-cost devices.' },
{ label: 'Explainable AI', desc: 'Engineered a transparent AI that visually explains its decisions to doctors, building clinical trust.' },
{ label: 'Adaptive Learning', desc: 'Designed a smart, adaptive system that continuously learns and improves using expert feedback.' },
{ label: 'End-to-End Ownership', desc: 'Managed the project from initial research to practical real-world application deployment.' },
],
...
'AI / ML': ['PyTorch', 'TensorFlow', 'YOLO', 'SBERT', 'FAISS', 'LLM APIs'],
'Tools': ['Docker', 'n8n', 'Firebase', 'Git', 'PowerBI', 'Flutter'],
...
.exp-skill-group:last-child {      margin-bottom: 0; }
    ...
    @media (max-width: 640px) {
      .exp-card-trigger { gap: 0.75rem; padding: 0.9rem 1rem 0.9rem 1.2rem; }
      .exp-logo-wrap { width: 38px; height: 38px; }
    }
  `
                    ...
                    >
                      <div className="exp-logo-wrap">
                        {exp.companyLogo ? (
                          ...
                          <span className="exp-logo-placeholder">{exp.icon}</span>
                        )}
                      </div>
                      ...
                      <div className="exp-card-info">
                        <div className="exp-role-row">
                          ...
                          <span className={`exp-badge-type exp-badge-${exp.type.toLowerCase()}`}>
                            {exp.type}
                          </span>
                        </div>
                    ...
                    <div className="exp-body">
                      <div className="exp-body-inner">
                        <div className="exp-body-pad">
                          ...
                          <ul className={`exp-bullets exp-bullets-${exp.color}`}>
                            {exp.bullets.map((b, j) => (
                              <li
                                key={j}
                                className="exp-bullet"
                                onMouseEnter={() => setHoveredBullet(`${i}-${j}`)}
                                onMouseLeave={() => setHoveredBullet(null)}
                              >
                                <span className={`exp-bullet-icon exp-bullet-icon-${exp.color}`}>✦</span>
                          ...
                          <div className="exp-tags">
                            {exp.tags.map(t => (
                              <span key={t} className={`exp-tag exp-tag-${exp.color}`}>{t}</span>
                            ))}
                          </div>
                        </div>
                      </div>
              ...
              {Object.entries(skillGroups).map(([group, chips]) => (
                <div key={group} className="exp-skill-group">
                  ...
                  <div className="exp-skill-chips">
                    {chips.map(c => (
                      <span key={c} className="exp-skill-chip">{c}</span>
                    ))}
                  </div>
                </div>
              ))}
